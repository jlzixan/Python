...
class DecisionTree:
    ...
    def fit(self, X, y):
        ...
        if self.max_depth <= 1 or splitModel.splitVariable is None:
            # If we have reached to the last level or when there is no splitting variable, just use the decision stump
        ...
        self.subModel1 = DecisionTree(self.max_depth - 1, stump_class=self.splitModel)
        self.subModel1.fit(X[splitIndex1,], y[splitIndex1])
        self.subModel0 = DecisionTree(self.max_depth - 1, stump_class=self.splitModel)
    ...
    def predict(self, X):
        ...
        if splitVariable is None:
            # Return majority label is there is not split
